...
CREATE OR REPLACE FUNCTION public.get_proposal_by_token(p_token text, p_email text DEFAULT NULL::text)
 RETURNS TABLE(proposal_id uuid, title text, description text, recipient_name text, recipient_email text, recipient_company text, items jsonb, total_hours numeric, total_value numeric, status text, valid_until date, created_at timestamp with time zone, template_content text, template_sections jsonb, admin_name text, admin_company text)
 LANGUAGE plpgsql
 SECURITY DEFINER
