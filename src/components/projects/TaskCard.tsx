...
export const TaskCard: React.FC<TaskCardProps> = ({
  ...
  const getStageInfo = (status: string) => {
    ...
    switch (status) {
      case "pending":
        return { name: "Pendente", color: "#eab308" };
      case "in_progress":
        return { name: "Em Andamento", color: "#3b82f6" };
      case "completed":
        return { name: "Concluída", color: "#22c55e" };
      default:
        return { name: status, color: null };
    }
  };
                ...
                <Tooltip>
                  ...
                  <TooltipContent>
                    <p>
                      ...
                      {format(parseISO(task.due_date), "dd 'de' MMMM", {
                        locale: ptBR,
                      })}
                    </p>
                  </TooltipContent>
        ...
        {timeEntries.length > 0 && (
          ...
          >
            <CollapsibleTrigger className="flex items-center gap-2 text-xs text-muted-foreground hover:text-foreground w-full">
              <ChevronDown
                className={`w-3 h-3 transition-transform ${entriesOpen ? "rotate-180" : ""}`}
              />
              {timeEntries.length}{" "}
              {timeEntries.length === 1 ? "registro" : "registros"} de horas
            </CollapsibleTrigger>
