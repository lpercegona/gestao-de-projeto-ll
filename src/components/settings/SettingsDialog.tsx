...
import { Globe, User, Lock, Users, Palette, Bell, Loader, History, Inbox } from "lucide-react";
...
import { RequestsSettingsTab } from "@/components/settings/RequestsSettingsTab";
...
export const SettingsDialog: React.FC<SettingsDialogProps> = ({ open, onOpenChange }) => {
  ...
  const navSections: NavSection[] = [
    ...
    ...(showAdminSections
      ? [
          ...
          { id: "requests", label: "Solicitações", icon: Inbox, adminOnly: true },
  ...
  const renderContent = () => {
    switch (activeSection) {
      ...
      case "requests":
        return showAdminSections ? <RequestsSettingsTab /> : null;
      case "activity-log":
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[95vw] sm:max-w-[768px] h-[80vh] max-h-[95vh] p-0 gap-0 overflow-hidden rounded-xl [&>button]:sm:bg-transparent [&>button]:sm:rounded-sm [&>button]:bg-muted/60 [&>button]:rounded-full [&>button]:w-4 [&>button]:h-4 [&>button]:flex [&>button]:items-center [&>button]:justify-center">
